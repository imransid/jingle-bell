...
const accessEnv = (key: string, defaultValue?: string | number): string => {
  ...
  cache[key] = Config[key] as string; // Use type assertion to tell TypeScript it's a string

  return Config[key] as string; // Use type assertion to tell TypeScript it's a string
};
